...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    answer_text = serializers.CharField(max_length=200, allow_null=True)
...
class QuestionSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if question_type == 'one' or question_type == 'multiple' or question_type == 'text':
            return attrs
        raise serializers.ValidationError('Тип вопроса может быть только один, несколько или текстовой')
...
class SurveySerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        return Survey.objecs.create(**validated_data)
